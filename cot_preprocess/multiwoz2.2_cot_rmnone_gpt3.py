...
def preprocess(sample_idx, dial_json, schema, out, idx_out, excluded_domains, frame_idxs, split):
    ...
    #
    gpt_explans = []
    # if os.path.exists(f"{data_dir}/{split}_gpt3_explains.json"):
    #     with open(f"{data_dir}/gpt3-results.{split}.txt") as exf:
    #         for line in exf.readlines():
    #             if line[0:-1] == "NONE":
    #                 gpt_explans.append(None)
    #             else:
    #                 gpt_explans.append(json.loads(line)["res"])
    if os.path.exists(f"{data_dir}/{split}_gpt3_explains.json"):
        with open(f"{data_dir}/{split}_gpt3_explains.json") as exf:
            gpt_explans = json.load(exf)
                ...
                for domain in schema:
                    ...
                    for slot in slots:
                        ...
                        if slot["name"] in active_slot_values.keys():
                            ...
                            chain_of_thought = ""
                            if len(gpt_explans) > 0 and gpt_explans[sample_idx] != 'NONE': 
                                chain_of_thought += gpt_explans[sample_idx]
