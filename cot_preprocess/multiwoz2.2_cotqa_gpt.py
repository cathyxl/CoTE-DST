...
def preprocess(sample_idx, dial_json, schema, out, idx_out, excluded_domains, frame_idxs, split):
    data_dir = "/".join(dial_json.split("/")[:-2])
    print(data_dir)
    ...
    # load explanations
    gpt_explans = []
    if os.path.exists(f"{data_dir}/{split}_gpt_explanations.json"):
        with open(f"{data_dir}/{split}_gpt_explanations.json") as exf, open(f"{data_dir}/{split}_mappings.json") as mapf:
            mappings = json.load(mapf)
            for line in exf.readlines():
                gpt_explans.append(json.loads(line))
    print(split, len(gpt_explans))
    # sample_idx = 0
                ...
                for domain in schema:
                    ...
                    for slot in slots:
                        ...
                        if slot["name"] in active_slot_values.keys():
                            ...
                            chain_of_thought = ""
                            if len(gpt_explans) > 0: 
                                chain_of_thought += "because "
                                first_char = gpt_explans[mappings[sample_idx]]["result"][0].lower()
                                chain_of_thought += first_char+gpt_explans[mappings[sample_idx]]["result"][1:-1]
                            else:
