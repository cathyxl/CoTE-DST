...
class DataTrainingArguments:
    ...
    early_stop: Optional[int] = field(
        default=0,
        metadata={
            "help": "whether to use early stop in trainer"
        },
    )
    patience: Optional[int] = field(
        default=3,
        metadata={
            "help": "num of epochs wait"
        },
    )
...
def main():
    ...
    if training_args.do_train:
        ...
        if data_args.max_train_samples is not None:
            # add by xl
            # set the range as a random list
            train_ids = random.choices(range(len(train_dataset)), k=data_args.max_train_samples)
            train_dataset = train_dataset.select(train_ids) # if seed is the same, the list is the same
    ...
    if data_args.early_stop == 1:
        early_stop_call = EarlyStoppingCallback(data_args.patience, 0.0)
        callbacks.append(early_stop_call)
